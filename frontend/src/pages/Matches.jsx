...
export default function Matches() {
  ...
  useEffect(() => {
    fetchMatches();
    if (searchParams.get('newMatch') === '1') {
      setShowModal(true);
    }
  }, []);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Match Registry</h1>
          <p className="text-sm text-purple-300/70 mt-1">Manage fixtures, line-ups, and live match tags</p>
        </div>
            ...
            <div key={match.id} className="glass-card p-6 space-y-5 flex flex-col justify-between hover:border-purple-500/50 transition-all">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-purple-400/60 font-mono font-medium">#{match.id}</span>
                ...
                <div className="flex items-center justify-between text-center py-2 bg-black/20 rounded-xl p-3 border border-purple-500/10">
                  <div>
                    <div className="font-bold text-white text-base">{match.team?.name || 'Home'}</div>
                    <div className="text-xs text-purple-400/60 font-medium">Home</div>
                  </div>
                  <div className="text-2xl font-black text-purple-400 px-4">
                    {match.status === 'finished' ? `${match.score_team} - ${match.score_opponent}` : 'VS'}
                  </div>
                  <div>
                    <div className="font-bold text-purple-200 text-base">{match.opponent_name}</div>
                    <div className="text-xs text-purple-400/60 font-medium">Away</div>
                  </div>
                </div>
                ...
                <div className="space-y-2 text-xs text-purple-300/70">
                  <div className="flex items-center gap-2">
                    ...
                    <span>{match.match_date ? new Date(match.match_date).toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) : 'TBD'}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <MapPin className="w-3.5 h-3.5 text-purple-400" />
                    <span>{match.location || 'GOR Serbaguna'}</span>
                  </div>
                  <div className="flex items-center gap-2">
      ...
      {showModal && (
        <div className="modal-overlay">
          <div className="modal-box">
            <h2 className="text-xl font-bold text-white mb-4">Schedule New Fixture</h2>
            <form onSubmit={handleCreateMatch} className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="GOR Serbaguna, Lapangan 1"
